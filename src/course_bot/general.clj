  ...
  (talk/def-talk db "restart"
    ...
    (fn [tx {{id :id} :from text :text} {stud-id :restart-stud}]
      (let [lower-text (str/lower-case text)]
        (cond
          (= lower-text "yes") (do (talk/send-text token id (str (tr :general/restarted-and-notified) stud-id))
                                   (talk/send-text token stud-id (str (tr :general/use-start-once-more)))
                                   (-> tx
                                       (codax/assoc-at [stud-id :allow-restart] true)
                                       (talk/stop-talk)))
          (= lower-text "no") (do (talk/send-text token id (tr :general/not-restarted))
                                  (talk/stop-talk tx))
