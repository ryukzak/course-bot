  ...
  {:en
   {:essay
    {:submit                           "Submit "
     ...
     :thank-you-your-essay-submitted    "Thank you, the text has been uploaded and will be submitted for review soon."
     :status                           " status"
     ...
     :duplicate-text                   "You've submitted the text which is very like to the text of other student. Please, send an original one."}}
   :ru
   {:essay
    {:submit                           "Отправить "
     ...
     :thank-you-your-essay-submitted    "Спасибо, текст загружен и скоро попадёт на рецензирование."
     :status                           " статус"
     ...
     :duplicate-text                   "Вы отправили текст, который очень похож на текст другого студента. Пожалуйста, отправьте оригинальный текст."}}})

(defn jaccard-similarity [s1 s2]
  (let [set1 (set s1)
        set2 (set s2)]
    ...
    (talk/def-talk db cmd help
                   ...
                   (fn [tx {{id :id} :from text :text}]
                     (let [essays (->> (get-essays tx essay-code)
                                       (map (fn [[_k v]] (-> v :essays (get essay-code) :text))))
                           similarity-threshold 0.9
                           is-duplicate? (some #(>= (jaccard-similarity text %) similarity-threshold) essays)]
                       (if is-duplicate?
                         (do
                           (talk/send-text token id (tr :essay/duplicate-text))
                           (talk/stop-talk tx))
                         (do
                   ...
                   (fn [tx {{id :id} :from text :text} {essay-text :essay-text}]
                     (talk/when-parse-yes-or-no
                       tx token id text
                       (talk/send-text token id (tr :essay/thank-you-your-essay-submitted))
                       (-> tx
